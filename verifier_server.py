...
class OnlineGateHGB:
    """
    Per-device online gate inference:
      - build ratio features from raw counters
      - baseline z-score normalization learned online from "very safe" windows (p<0.05)
      - rolling aggregation over last W windows -> HGB predict_proba

    Expects joblib blob format:
      {
        "model": fitted sklearn model (HGB),
        "W": int,
        "thr": float,
        "base_features": [...],
        "agg_features": [...],
    ...
    def __init__(self, blob: dict, baseline_warmup_n: int = 20, warmup_s: float = 10.0, min_baseline_n: int = 20):
        ...
        # --- baseline warmup control ---
        self.warmup_s = float(warmup_s)
        self.min_baseline_n = int(min_baseline_n)

        self.baseline_warmup_n = int(baseline_warmup_n)
        self.base_hist = deque(maxlen=self.baseline_warmup_n)
        self.baseline_frozen = False
        self._mu = None
        self._sd = None

        # NEW: trust-gated warmup timer
        self.trusted_since_ts: float | None = None

        # rolling inference window
        self.win_hist = deque(maxlen=self.W)

    def reset(self, reason: str = "reset"):
        # wipe everything that could make normalization drift
        self.base_hist.clear()
        self.win_hist.clear()
        self.baseline_frozen = False
        self._mu = None
        self._sd = None
        self.trusted_since_ts = None

    def on_trust_state(self, trust_state: str, now: float):
        # Start warmup timer only when TRUSTED
        if trust_state == "TRUSTED":
            if self.trusted_since_ts is None:
                self.trusted_since_ts = float(now)
        else:
            # if not trusted, baseline learning must not happen
            self.trusted_since_ts = None
    ...
    def _baseline_stats(self):
        # if we already computed it, reuse forever
        if self.baseline_frozen and (self._mu is not None) and (self._sd is not None):
            return (self._mu, self._sd)

        # collect baseline from first N windows only
        if len(self.base_hist) < self.baseline_warmup_n:
            return None

        X = np.asarray(self.base_hist, dtype=np.float32)
        mu = X.mean(axis=0)
    ...
    def update(self, w: dict, trust_state: str, now: float) -> Optional[dict]:
        """
        - If not TRUSTED: do NOT learn baseline, do NOT infer (return None)
        - If TRUSTED and baseline not frozen:
            * collect baseline samples during first warmup_s seconds since becoming TRUSTED
            * freeze when time passed AND we have enough samples
        - After baseline frozen:
            * do rolling W aggregation and output prediction
        """

        # gate on trust
        self.on_trust_state(trust_state, now)
        if trust_state != "TRUSTED":
            return None

        x_raw = np.asarray(self._base_vec_from_window(w), dtype=np.float32)

        # ---- baseline learn/freeze phase ----
        if not self.baseline_frozen:
            # If we don't have a trusted_since, we can't warmup safely
            if self.trusted_since_ts is None:
                return None

            # collect baseline sample
            self.base_hist.append(x_raw.copy())

            # freeze only after warmup_s AND enough samples
            dt = float(now) - float(self.trusted_since_ts)
            if dt < self.warmup_s or len(self.base_hist) < self.min_baseline_n:
                return None
        ...
        # ---- apply baseline z-score ----
        #x = (x_raw - mu) / (sd + 1e-6)
        x = x_raw

        # ---- rolling W aggregation ----
...
class VerifierPolicyServer:
    ...
    async def attest_partial_and_log(self, dev: str, k: int, trigger: str = "POLICY", ml: Optional[dict] = None) -> dict:
        async with self._attest_lock(dev):
            ...
            if isinstance(resp, dict) and resp.get("verify_ok", False):
                # RESET HGB baseline on any PARTIAL failure
                ge = self.gate_hgb_engines.get(dev)
                if ge is not None:
            ...
            if isinstance(resp, dict) and not bool(resp.get("verify_ok", False)):
                ...
                for k in ("budget_reason", "budget_cost_units", "budget_tokens_after", "budget_tokens_before",
    ...
    async def policy_loop(self, dev: str):
        ...
        while True:
                        ...
                        if ML_ENABLE and windows:
                            ...
                            for w in windows:
                                ...
                                if ge is not None:
                                    ...
                                    trust_state_now = (self.devices.get(dev).trust_state if self.devices.get(dev) else TRUST_UNKNOWN)
                                    gout = ge.update(w, trust_state=trust_state_now, now=time.time())
                                    ...
                                    if isinstance(gout, dict):
                                        ...
                                        # HGB exists but not ready yet (baseline/W warmup)
                                        g_src = "HGB_WARMUP"
            ...
            if decision.attest_kind != PLAttestKind.NONE:
                ...
                # ---- HARD OVERRIDE: COMPROMISED + INJECTION => FULL without budget gate ----
                bypass_budget = False
                try:
                ...
                if bypass_budget:
                    ...
                    # (προαιρετικό) log στο events
                    if fp_evt:
                        self._jwrite(fp_evt, {
                            "ts_ms": ts_ms(),
                            "device": dev,
                            "event": "budget_bypass",
                            "reason": "COMPROMISED+INJECTION => FULL without budget gate",
                        })

                    asyncio.create_task(self.attest_full_and_log(dev, trigger="POLICY_BYPASS_BUDGET", ml=ml2))
                    await asyncio.sleep(LOOP_TICK_S + random.random() * JITTER_S)
                    continue
    ...
    async def handle_client(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        ...
        if ML_ENABLE and self.gate_hgb_blob is not None:
           self.gate_hgb_engines[device_id] = OnlineGateHGB(
                self.gate_hgb_blob,
                baseline_warmup_n=200,   # δώσε “χώρο” για δείγματα, δεν πειράζει
                warmup_s=10.0,           # 10 seconds warmup
                min_baseline_n=20        # minimum samples before freeze
            )
