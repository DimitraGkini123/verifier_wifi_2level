...
#quarantine configs
QUARANTINE_ON_FULL_FAILS = 100**9         # trigger after 2 consecutive FULL failures
...
WL_PATH   = "models/workload_retrained_hgb_compatible.joblib"
ATK_PATH  = "models/attacktype_lr_compromised_only.joblib"
ML_ENABLE = True
...
class VerifierPolicyServer:
    ...
    async def _quarantine_periodic_recheck(self, dev: str):
        ...
        if ok:
            ...
            # optional: reset policy state so it doesn't keep stale history
            try:
    ...
    def _load_budget_config(
        ...
        # NEW: preload per-level coverage if provided
        ...
        for dev, level_name in devices.items():
            ...
            # NEW: if coverage not specified for this level, give a default
            ...
            if isinstance(raw, dict):
                for dev, st in raw.items():
                    if isinstance(st, dict):
                        self.block_lru[dev] = DeviceLRUBlocks.from_state(st)  # determ
        except FileNotFoundError:
            pass
        except Exception:
            pass
    ...
    async def policy_loop(self, dev: str):
        # Wait until FULL golden exists (provisioning may be async)
        ...
        # Initial FULL attest (optional) - bypass budget (baseline)
        ...
        while True:
            ...
            # Tick policy (may be updated later after ML inference)
            decision = self.policy.tick(dev, now=now)

            # =========================
            # QUARANTINE MODE FLAG
            # =========================
            in_quarantine = bool(getattr(dc, "quarantined", False))

            # NOTE: We do NOT "continue" here anymore.
            # In quarantine: WINDOWS continue, POLICY-DRIVEN ATTEST is blocked,
            # and we do periodic FULL rechecks.

            # =========================
            # 1) GET_WINDOWS (if due)  <-- NOW RUNS EVEN IN QUARANTINE
            # =========================
                    ...
                    if resp.get("type") == "WINDOWS":
                        ...
                        if self.lr_policy is not None and windows:
                            ...
                            for w in windows:
                                ...
                                g = (pr.get("gate_label") or "UNCERTAIN").upper()
                                if g not in ("SAFE", "COMPROMISED", "UNCERTAIN"):
                                    g = "UNCERTAIN"
                                g_enum = GateLabel(g)
                                ...
                                if g_enum == GateLabel.COMPROMISED:
                                    atk = (pr.get("attack2b_label") or "UNKNOWN").upper()
                                    if atk not in ("CODE_INJECTION", "INTERRUPTION", "ROP", "UNKNOWN"):
                                        atk = "UNKNOWN"
                                    atk_enum = Attack2BLabel(atk)
                                    atk_labels.append(atk_enum)
                                    atk_counts[atk] = atk_counts.get(atk, 0) + 1

                                    ac = pr.get("attack2b_conf")
                                    if ac is not None:
                                        try:
            ...
            # =========================
            # QUARANTINE: periodic FULL recheck (does NOT block windows)
            # =========================
            if in_quarantine:
                tnow = time.time()
                if (tnow - float(dc.last_quarantine_check_ts or 0.0)) >= QUARANTINE_RECHECK_S:
                    ...
                    resp = await self.attest_full_and_log(
                        dev,
                        trigger="QUARANTINE_RECHECK",
                        ml={"policy_reason": "quarantine_recheck"}
                    )
            ...
            #    In quarantine: SKIP policy attest (but windows still run)
            # =========================
            if (not in_quarantine) and (decision.attest_kind != PLAttestKind.NONE):
                st = self.policy.devices.get(dev)
                ...
                fit_kind, fit_k, cost_units, budget_reason = self.budget.fit_plan(
                    ...
                    ideal_k=ideal_k,
                    block_count=max(1, real_bc if real_bc > 0 else 1),  # costing only
                    min_k=min_k
                )
            ...
            elif in_quarantine and (decision.attest_kind != PLAttestKind.NONE) and fp_evt:
                # Explicit log that we skipped policy attest due to quarantine
                st = self.policy.devices.get(dev)
                self._jwrite(fp_evt, {
                    "ts_ms": ts_ms(),
                    "device": dev,
    ...
    async def handle_client(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        ...
        # helper: robust close on Windows (WinError 121 etc.)
